...
A 4-page analysis, pared down from the standalone RETITC analysis PDF
generated by ~/Census-Forecaster/generate_reec_report.py. It keeps the four
charts that carry a finding, drops the glossary and both data tables, and folds
Executive Order 26-02 — the governor's one-year exemption from Act 24's $40M
cap, which post-dates the PDF — onto one page.

It describes and does not advise: the recommendations page that briefly lived
here was removed at the user's direction (git history, ed13dfe). The prose is
written for a reader who does not do this for a living — every term of art is
glossed where it first appears, because there is no glossary page to send them
to.
...
def stat(key: str) -> str:
    """One of page 5's three fiscal tiles — what the savings chart and the
    fiscal table used to say between them."""
    return (f'<div class="stat"{L.attr(f"saves.stat.{key}")}>'
            f'<div class="stat-n">{C.t(f"saves.stat.{key}.n")}</div>'
            f'<div class="stat-l">{C.t(f"saves.stat.{key}.l")}</div></div>')
